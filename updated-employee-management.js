...
export default {
  ...
  methods: {
    ...
    async fetchEmployees() {
      try {
        console.log('🔍 Fetching employees for current branch manager...')
        ...
        if (data.success) {
          ...
          console.log('✅ Employees loaded:', {
            ...
            branchFilter: data.filters?.branch_id,
            userType: data.filters?.user_type
          })

          // Show branch filtering info to user
          if (data.filters && data.filters.branch_id) {
            this.$emit(
              'show-snackbar', 
              `Showing ${this.employees.length} employees from your branch (ID: ${data.filters.branch_id})`, 
              'info',
              3000
            )
          }
        ...
        if (error.message.includes('401') || error.message.includes('unauthorized')) {
          this.$emit('show-snackbar', 'Session expired. Please login again.', 'warning')
